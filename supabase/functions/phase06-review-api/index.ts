...
async function resolveReviewer(authorization: string) {
  ...
  if (!staffRes.ok) {
    return { ok: false as const, status: 401, error: "STAFF_SESSION_INVALID" };
  }
  ...
  if (!email) {
    return { ok: false as const, status: 403, error: "STAFF_EMAIL_REQUIRED" };
  }
  ...
  for (let page = 1; page <= 10 && !authUserId; page++) {
    ...
    if (error) {
      return { ok: false as const, status: 500, error: "AUTH_LOOKUP_FAILED" };
    }
  ...
  if (!authUserId) {
    return { ok: false as const, status: 403, error: "REVIEWER_AUTH_USER_NOT_FOUND" };
  }
  ...
  if (reviewerError) {
    return { ok: false as const, status: 500, error: "REVIEWER_AUTHORIZATION_CHECK_FAILED" };
  }
  if (!allowed) {
    return { ok: false as const, status: 403, error: "REVIEWER_NOT_AUTHORIZED" };
  }

  return { ok: true as const, reviewerId: authUserId, email, staff };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "queue") {
      const level = text(body?.level, 2);
      const skill = text(body?.skill, 3);
      ...
      const { data, error } = await admin.rpc("phase06_get_review_queue", {
        p_cefr_level: level,
        p_skill_code: skill,
        p_status: text(body?.status, 40),
        p_release_code: text(body?.release, 60),
        p_limit: limit,
        p_offset: offset,
      });
      if (error) throw error;
    ...
    if (action === "item") {
      const itemVersionId = text(body?.itemVersionId, 80);
      if (!itemVersionId) return json({ error: "ITEM_VERSION_ID_REQUIRED" }, 400, origin);
      const { data, error } = await admin.rpc("phase06_get_review_item", {
    ...
    if (action === "neighbor") {
      ...
      if (!new Set(["NEXT", "PREVIOUS"]).has(direction)) {
        return json({ error: "INVALID_DIRECTION" }, 400, origin);
      }
    ...
    if (action === "submit") {
      ...
      const qaGate = text(body?.qaGate, 80);
      ...
      if (!itemVersionId || !qaGate || !visibleActions.has(actionCode)) {
        return json({ error: "INVALID_REVIEW_SUBMISSION" }, 400, origin);
      }
